...
def get_args():
    ...
    base_ckpt_path = '/ceph/hpc/data/b2025b05-014-users/haiping_env/point-mae-pretrain'
    args.experiment_path = os.path.join(base_ckpt_path,'experiments', Path(args.config).stem, Path(args.config).parent.stem, args.exp_name)
    args.tfboard_path = os.path.join(base_ckpt_path,'experiments', Path(args.config).stem, Path(args.config).parent.stem,'TFBoard' ,args.exp_name)
